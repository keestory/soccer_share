...
## ✅ 2차 조치 완료 (Tier 2 일부 + Tier 3 알림)

- **알림/인박스(연결 레이어)** — `Notification` 모델 + `/notifications` 인박스 + Nav 벨 배지. 게임 성사·참가·취소·댓글 시 수신자에게 알림 생성. 매칭 앱의 리텐션 복귀 트리거 구현. (교차합의 #1 일부)
- **과거/취소 게임 필터** — `/games`는 임박한 경기만(matchDate ≥ 오늘), 취소 제외, 날짜순 정렬
- **예약 날짜 UTC off-by-one 수정** — 로컬 기준 YYYY-MM-DD 생성
- **스탯 통합 기반** — `Player.userId`를 실제 관계로 승격(User↔로스터 Player). (표시/연결 UI는 다음 단계)

## ⏳ 남은 항목

**Tier 2 (엔지니어링) — 이 환경 제약으로 보류**
- SQLite → **PostgreSQL** 마이그레이션(+커넥션 풀링, 예약 exclusion 제약, 실제 enum): 이 환경에 Postgres가 없어 테스트 불가 → 마이그레이션 가이드로 남김. 현재는 앱단 검증(클램프·화이트리스트·트랜잭션)으로 완화.
- 자동화 테스트/ESLint 도입, 리더보드 집계를 materialized stats 테이블로(대규모 대비)

**✅ 3차 조치 완료**
- **콜드스타트 1차 — 지역 새 게임 알림 구독**: `RegionSubscription` + /games 구독 토글, 새 게임 생성 시 그 지역 구독자에게 알림. (수요 집계 → 공급 발생 시 복귀 트리거)
- **스탯 통합 완료**: 로스터 Player↔회원 연결 UI + 프로필에 팀 경기 기록 표시(픽업 기록과 함께)

**✅ 4차 조치 완료**
- **리더보드 스탯 통합**: 픽업 + 팀 로스터 기록을 유저별 단일 랭킹으로 통합 집계
- **MVP 참가자 투표**: 주최자 지정 → `GameVote`(1인 1표) 최다 득표 자동 반영

**Tier 3 — 남은 결정 항목 (의도적 보류)**
- **시장/결제 정합성**: region/currency/date 시장별 데이터화(Market 설정), 결제 재도입·현지 레일. ⚠️ Business 리뷰가 *"한 동네 fill+repeat 검증 전엔 region 탈하드코딩에 투자하지 말라"*고 명시 → *"한국 MVP 학습 vs Indonesia 진입"* 결정 **선행 필요**. 결정 전 리팩터는 리뷰 권고에 반함.
- **추가 콜드스타트**: 초대/추천 링크, 한 동네 수동 운영 검증(Business 최우선 권고 — 코드가 아닌 실행)

**환경 제약으로 보류 (가이드만 남김)**
- SQLite→PostgreSQL 마이그레이션, 실제 enum, 자동화 테스트/ESLint: 이 환경에 Postgres 부재. 앱단 방어(클램프·화이트리스트·트랜잭션)로 완화.
