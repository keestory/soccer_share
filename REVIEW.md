...
**Tier 3 (제품/전략 결정)**
- **콜드스타트**: 초대/추천 링크 + "우리 동네 게임 알림" 구독 (알림 인프라 위에 확장). Business 권고 = 앱 확장 전 한 동네 수동 검증 우선
- **스탯 통합 마무리**: 로스터 Player↔User 연결 UI + 프로필에 로스터 스탯 표시 + 리더보드 통합, MVP 참가자 투표
- **시장/결제 정합성**: region/currency/date 시장별 데이터화(Market 설정), 결제 재도입·현지 레일
