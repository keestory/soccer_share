...
**Tier 3 — 남은 결정 항목**
- **스탯 통합 잔여**: 글로벌 리더보드에 로스터 스탯 통합(현재 픽업 기반), MVP 참가자 투표(현재 주최자 지정)
- **시장/결제 정합성**: region/currency/date 시장별 데이터화(Market 설정), 결제 재도입·현지 레일 — *"지금 한국 MVP로 학습 vs Indonesia 진입"* 결정 선행 필요(가장 큰 전략적 갈림길)
